 ...
 * @details Sends org.rdk.HdmiCecSource.setOSDName with params name "Sky TV" and compares the
 ...
 *          the result body.
 *
 *          THIS WRITE IS POSITION 15's PRECONDITION. The plugin's provisioned OSD name is "TV
 *          Box", so "Sky TV" becomes observable through getOSDName only because this position
 *          writes it; the plugin also persists the value under cecOSDName. The module name
 *          says OTP where the API it drives is OSD - the name is left exactly as it is because
 *          renaming a registered module would remove it from the suite, which is not
 *          permitted.
 *
 * @precondition
